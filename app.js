...
try {
    ...
    if (customerEmail && (proParam === 'success' || proParam === 'active' || orderId)) {
        ...
        if (emailRegex.test(customerEmail)) {
            ...
            import('./js/supabase-georide.js')
                .then(({ checkProStatusByEmail }) => {
                    checkProStatusByEmail(customerEmail)
                        .then((res) => {
                            if (!res || !res.is_pro) {
                                setProStatus(false);
                                updateToggleStates();
...
// Expose globally
window.map = map;
window.THREE = typeof THREE !== 'undefined' ? THREE : window.THREE;
...
map.on('load', () => {
    ...
    // Accessibility: Mapbox renders its attribution button dynamically - patch aria-label
