/**
 * @file three-manager.js
 * @description Three.js 3D rendering pipeline for GEO Ride, managing vehicle meshes, wheel rotation, skidmark line geometry, lighting presets, and WebGL resource disposal.
 */
...
let threeLayer;
let vehicleMarker;
const THREE_STATIC = typeof window !== 'undefined' ? window.THREE : null;

function getThree() {
    return typeof window !== 'undefined' && window.THREE ? window.THREE : THREE_STATIC;
}

/**
 * Recursively disposes Three.js geometries, materials, and textures from a node hierarchy to prevent memory leaks.
 * @param {Object} node - Three.js Object3D / Mesh node.
 * @returns {void}
 */
...
/**
 * Returns active Mapbox custom Three.js layer instance.
 * @returns {Object} Three.js layer instance.
 */
...
/**
 * Returns active 2D vehicle marker instance.
 * @returns {Object} Mapbox 2D marker.
 */
export function getVehicleMarker() {
    return vehicleMarker;
}

/**
 * Re-creates or updates 2D vehicle marker overlay on Mapbox map.
...
export function setupVehicleMarker(map) {
    ...
    if (!state.is3D || state.activeVehicle === 'god') {
        ...
        vehicleMarker = new mapboxgl.Marker({ element: el, rotationAlignment: 'map', pitchAlignment: 'map' })
            ...
            .setRotation(state.bearing)
            .addTo(map);
    }
}

/**
 * Instantiates or updates 3D custom layer on Mapbox map.
 * @param {Object} map - Mapbox GL JS map instance.
 * @returns {void}
 */
export function setup3DVehicleLayer(map) {
    ...
    threeLayer = {
        ...
        buildVehicle: function (type) {
            const THREE = getThree();
            if (!THREE) return;
            while (this.vehicleGroup && this.vehicleGroup.children.length > 0) {
            ...
            if (type === 'car' || type === 'god') {
                ...
                const fender = new THREE.Mesh(
                    new THREE.BoxGeometry(2.4, 0.4, 2.2),
                ...
                [
                    ...
                    handle.position.set(pos[0] + (pos[0] < 0 ? 0.3 : -0.3), 1.8, -4.04);
                    this.vehicleGroup.add(handle);
                });
                ...
                const glass = new THREE.Mesh(
                    new THREE.BoxGeometry(2.1, 0.8, 0.1),
                    new THREE.MeshStandardMaterial({
                        color: 0x00f2ff,
                        transparent: true,
                        opacity: 0.6,
            ...
            [
                ...
                const b = new THREE.Mesh(brakeGeom, this.brakeMat);
                b.position.set(...pos);
                this.vehicleGroup.add(b);
            });
        },
        render: function (gl, matrix) {
            ...
            if (state.isTeleporting) {
                const progress = Math.max(
                    0,
                    Math.min(1, (Date.now() - (state.teleportStartTime || 0)) / (state.teleportDuration || 1000))
                );
            ...
            this.translation
                ...
                .scale(this.scaleVec)
                .multiply(this.rotation.makeRotationX(this.modelTransform.rotateX + targetPitch))
                .multiply(this.rotation.makeRotationY(angleRad))
                .multiply(this.rotation.makeRotationZ(targetRoll));
...
export function updateSkidMarks(map) {
    ...
    if (shouldMark) {
        ...
        const sideDist = state.activeVehicle === 'bus' ? 0.000045 : state.activeVehicle === 'truck' ? 0.00006 : 0.00004;
        const rearDist =
            state.activeVehicle === 'bus' ? -0.00005 : state.activeVehicle === 'truck' ? -0.000155 : -0.00001;
