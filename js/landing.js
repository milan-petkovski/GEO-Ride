...
let isFleetDragging = false, fleetPreviousMousePosition = { x: 0, y: 0 };
    ...
    function startThree() {
        ...
        window.addEventListener('mouseup', () => {
            if (isFleetDragging) {
                ...
                setTimeout(() => { fleetAutoRotate = true; }, 2500);
            }
        });
...
function buildFleet3DMesh(type) {
    ...
    const cyanNeonMat = new THREE.MeshStandardMaterial({ color: 0x00f2ff, emissive: 0x00f2ff, emissiveIntensity: 1.8, roughness: 0.1 });
    const glassMat = new THREE.MeshStandardMaterial({ color: 0x00f2ff, transparent: true, opacity: 0.65, roughness: 0.1 });
    const tireMat = new THREE.MeshStandardMaterial({ color: 0x04060a, roughness: 0.8 });
    const proMagentaMat = new THREE.MeshStandardMaterial({ color: 0xff00e5, emissive: 0xff00e5, emissiveIntensity: 2.2, wireframe: true });
    ...
    if (type === 'car') {
        ...
        const tailBar = new THREE.Mesh(new THREE.BoxGeometry(1.7, 0.08, 0.1), new THREE.MeshStandardMaterial({ color: 0xff1e42, emissive: 0xff1e42, emissiveIntensity: 2.2 }));
        ...
        [[-0.95, 0.38, 1.3], [0.95, 0.38, 1.3], [-0.95, 0.38, -1.3], [0.95, 0.38, -1.3]].forEach(pos => {
        ...
        wheelGeom.rotateZ(Math.PI / 2);
        [[-1.05, 0.42, 1.5], [1.05, 0.42, 1.5], [-1.05, 0.42, -1.2], [1.05, 0.42, -1.2], [-1.05, 0.42, -2.8], [1.05, 0.42, -2.8]].forEach(pos => {
            const w = new THREE.Mesh(wheelGeom, tireMat);
            w.position.set(...pos);
            fleetVehicleGroup.add(w);
        });
        ...
        wheelGeom.rotateZ(Math.PI / 2);
        [[-1.02, 0.4, 1.8], [1.02, 0.4, 1.8], [-1.02, 0.4, -1.8], [1.02, 0.4, -1.8]].forEach(pos => {
            const w = new THREE.Mesh(wheelGeom, tireMat);
            w.position.set(...pos);
            fleetVehicleGroup.add(w);
        });
